...
export const KNOWLEDGE_BANK = [
  ...
  {
    id: 'k-glassmorphism',
    elaboration: `<p>Glassmorphism simulates frosted glass, allowing the background to subtly bleed through the surface via a blur effect. When executed correctly, it adds a layer of premium, modern sophistication to an interface. When executed poorly, it destroys legibility and feels chaotic.</p><h3>The Rules of Glass</h3><p>Glassmorphism relies on the CSS <code>backdrop-filter: blur()</code> property, but blur alone is not enough. To sell the illusion of physical glass, you must include three elements:</p><ol><li><strong>The Blur:</strong> Usually between 12px and 24px.</li><li><strong>The Tint:</strong> A semi-transparent fill (e.g., <code>rgba(255,255,255, 0.6)</code> for light themes, or <code>rgba(0,0,0, 0.4)</code> for dark themes) to ensure the text on top remains readable regardless of what is underneath.</li><li><strong>The Edge:</strong> A 1px semi-transparent inner border (often white at 20% opacity) that catches the "light" and defines the physical edge of the glass pane.</li></ol><p>Reserve glassmorphism for floating elements like navigation bars, sticky headers, or modals. Never use it for dense data tables or primary reading content.</p>`,
    category: 'Depth & Elevation',
  ...
  },
  {
    ...
    elaboration: `<p>Shadows are not the only way to convey depth. Modern, "flat" or "clean" interfaces often rely on surface hierarchy—using subtle shifts in background lightness to differentiate overlapping planes.</p><h3>Light and Dark Surfaces</h3><p>In a light theme, the lowest base layer is typically a very light gray (e.g., <code>#F3F4F6</code>), while the cards resting on top are pure white (<code>#FFFFFF</code>). The contrast between the gray floor and the white card defines the boundary without needing a shadow.</p><p>In dark mode, the physics are inverted. Because you cannot cast a dark shadow on a dark background effectively, higher elevations must be communicated by making the surface <em>lighter</em>. The base layer might be <code>#121212</code>, a card resting on it might be <code>#1E1E1E</code>, and a dropdown floating above that might be <code>#2C2C2C</code>. This "lightness as elevation" principle is critical for building dark modes that don't feel flat and suffocating.</p>`,
    category: 'Depth & Elevation',
  ...
  },
  {
    ...
    elaboration: `<p>For the past decade, responsive design was dictated by the width of the viewport (the browser window). However, in modern component-based frameworks (like React or Vue), a component doesn't know where it will be placed. A "Product Card" might be placed in a wide main content area, or squeezed into a narrow sidebar.</p><h3>Micro-Responsiveness</h3><p>Container queries (<code>@container</code>) solve this by allowing a component to adapt its layout based on the size of its parent container, rather than the entire screen. If the card's container is wider than 400px, it can display a horizontal layout (image on the left, text on the right). If the container drops below 400px, it snaps to a stacked vertical layout.</p><p>This allows designers to build truly modular, "write once, place anywhere" components that intelligently adapt to their micro-environment, vastly improving the flexibility and robustness of the design system.</p>`,
    category: 'Responsive Design',
