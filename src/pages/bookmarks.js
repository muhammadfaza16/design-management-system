...
export async function renderBookmarks(container, navigate) {
  ...
  async function load() {
      ...
      container.innerHTML = `
        <div class="page animate-fade-in">
          ...
          <div class="lib-controls" style="margin-bottom:var(--space-6)">
            <div class="section-title" style="margin-bottom:0">Saved Links <span id="bookmark-count" style="font-weight:400;opacity:0.5"></span></div>
    ...
    // Dynamic Updates
    const countEl = container.querySelector('#bookmark-count');
    if (countEl) countEl.textContent = `(${bookmarks.length})`;

    container.querySelector('#bookmarks-grid').innerHTML = bookmarks.length === 0 ? `
