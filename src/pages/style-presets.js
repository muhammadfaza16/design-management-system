...
export async function renderStylePresets(container, navigate) {
  ...
  async function load() {
    ...
    container.querySelector('#style-add').addEventListener('click', async () => {
      const name = prompt('Preset name (e.g. "Dark SaaS Theme"):');
      if (!name) return;
      const description = prompt('Description:') || '';
      const colorsRaw = prompt('Colors (comma-separated hex, e.g. #000000, #ffffff, #3b82f6):') || '';
      const fontsRaw = prompt('Font families (comma-separated):') || '';
      const radius = prompt('Border radius base (e.g. 8px, 12px):') || '';

      const colors = colorsRaw.split(',').map(c => c.trim()).filter(Boolean);
      const fonts = fontsRaw.split(',').map(f => f.trim()).filter(Boolean);

      await addStylePreset({ name, description, tokens: { colors, fonts, radius } });
